...
class pages_elementor extends Widget_Base {
	...
	protected function render() {
		...
		$tag        = in_array( $settings['pages_title_tag'], $valid_tags ) ? $settings['pages_title_tag'] : 'h5';
		...
		$default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
			...
			esc_attr( $tag  ),
			$icon_enabled,
			esc_attr( $settings['pages_title_icon']),
			esc_attr( $settings['pages_icon_position']),
			'mdp-widgeter-elementor-list-align-' . esc_attr( $settings['pages_list_alignment'] )
		);

		$title = str_replace( [ '<', '>', '&', '"', '”', '\'', '‘' ], '', $settings['pages_title'] );
