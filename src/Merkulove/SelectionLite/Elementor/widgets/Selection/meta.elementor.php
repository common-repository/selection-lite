...
class meta_elementor extends Widget_Base {
	...
	protected function render() {
		...
		$tag        = in_array( $settings['meta_title_tag'], $valid_tags ) ? $settings['meta_title_tag'] : 'h5';

		$default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
			'meta',
			esc_attr( $tag ),
			$settings['enable_meta_title_icon'] === 'yes',
			esc_attr( $settings['meta_title_icon'] ),
			esc_attr( $settings['meta_icon_position'] ),
			'mdp-widgeter-elementor-list-align-' . esc_attr( $settings['meta_list_alignment'] )
		);

		$title = str_replace( [ '<', '>', '&', '"', '”', '\'', '‘' ], '', $settings['meta_title'] );
