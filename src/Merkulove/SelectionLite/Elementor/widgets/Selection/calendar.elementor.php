...
class calendar_elementor extends Widget_Base {
	...
	protected function render() {
		...
		$tag        = in_array( $settings['calendar_title_tag'], $valid_tags ) ? $settings['calendar_title_tag'] : 'h5';

		$default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
			'calendar',
			esc_attr( $tag ),
			$settings['enable_calendar_title_icon'] === 'yes',
			esc_attr( $settings['calendar_title_icon'] ),
			esc_attr( $settings['calendar_icon_position'] )
		);

		$title = str_replace( [ '<', '>', '&', '"', '”', '\'', '‘' ], '', $settings['calendar_title'] );
