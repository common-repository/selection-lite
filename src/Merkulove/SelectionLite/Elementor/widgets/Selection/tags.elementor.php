...
class tags_elementor extends Widget_Base {
	...
	private function generate_typography_tabs_controls( $section_id, $opts = [] ) {
		...
		if ( $style_opts['include_typography'] ) {
			...
			if ( $style_opts['exclude_font_size'] ) {
				$typography_settings['exclude'] = [ 'font_size' ];
			}
	...
	protected function render() {
		...
		$tag        = in_array( $settings['tag_cloud_title_tag'], $valid_tags ) ? $settings['tag_cloud_title_tag'] : 'h5';
		...
		$default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
			'tag-cloud',
			esc_attr( $tag ),
			$settings['enable_tag_cloud_title_icon'] === 'yes',
			esc_attr( $settings['tag_cloud_title_icon'] ),
			esc_attr( $settings['tag_cloud_icon_position'] ),
			'',
			$before_widget
		);

		$title = str_replace( [ '<', '>', '&', '"', '”', '\'', '‘' ], '', $settings['tag_cloud_title'] );
