...
class comments_elementor extends Widget_Base {
	...
	protected function render() {
		...
		$tag        = in_array( $settings['comments_title_tag'], $valid_tags ) ? $settings['comments_title_tag'] : 'h5';

		$default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
			'comments',
			esc_attr( $tag ),
			$settings['enable_comments_title_icon'] === 'yes',
			esc_attr( $settings['comments_title_icon'] ),
			esc_attr( $settings['comments_icon_position'] ),
			'mdp-widgeter-elementor-list-align-' . esc_attr( $settings['comments_list_alignment'] )
		);

		$title  = str_replace( [ '<', '>', '&', '"', '”', '\'', '‘' ], '', $settings['comments_title'] );
		$number = $settings['number_of_comments']['size'] ?? '0';
