...
class archives_elementor extends Widget_Base {
	...
	protected function render() {
		...
		$tag               = in_array( $settings['archive_title_tag'], $valid_tags ) ? $settings['archive_title_tag'] : 'h5';

		$default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
			'archives',
			esc_attr( $tag ),
			$settings['enable_archives_title_icon'] === 'yes',
			esc_attr( $settings['archives_title_icon']),
			esc_attr($settings['archives_icon_position']),
			'mdp-widgeter-elementor-list-align-' . esc_attr( $settings['archives_list_alignment'] )
		);

		$title = str_replace( [ '<', '>', '&', '"', '”', '\'', '‘' ], '', $settings['archives_title'] );
