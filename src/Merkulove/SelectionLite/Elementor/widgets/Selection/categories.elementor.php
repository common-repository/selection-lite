...
class categories_elementor extends Widget_Base {
	...
	protected function render() {
		...
		$tag        = in_array( $settings['categories_title_tag'], $valid_tags, true ) ? $settings['categories_title_tag'] : 'h5';

		$default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
			'categories',
			esc_attr( $tag ),
			$settings['enable_categories_title_icon'] === 'yes',
			esc_attr( $settings['categories_title_icon'] ),
			esc_attr( $settings['categories_icon_position']),
			'mdp-widgeter-elementor-list-align-' . esc_attr( $settings['categories_list_alignment'] )
		);

		$title = str_replace( [ '<', '>', '&', '"', '”', '\'', '‘' ], '', $settings['categories_title'] );
